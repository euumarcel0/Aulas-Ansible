...
# Instância Debian com Ansible
resource "aws_instance" "Debian_AnsiblePicPay" {
  ...
  user_data = <<-EOF
              #!/bin/bash
               ...
               hostnamectl set-hostname ServerHostPicPay
...
output "debian_ansible_instance_public_ip" {
  ...
  value       = aws_instance.Debian_AnsiblePicPay.public_ip
}